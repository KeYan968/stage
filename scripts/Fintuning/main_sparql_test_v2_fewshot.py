...
def build_fewshot_prompt(test_item, shot_examples):
    """
    V2 format: includes error_type for both examples and the test question.
    """
    examples_text = ""
...
# Select one example per error type for maximum coverage
def get_one_per_error_type(train_data, error_types, seed=RANDOM_SEED):
    """
    Returns one randomly selected example for each error type.
    """
    random.seed(seed)
    shot_examples = []
