"""
Few-shot inference — Fine-tuned V1 (no error_type).
...
Usage:
    python3 main_sparql_test_v1_fewshot_5runs.py
"""
...
OUTPUT_DIR   = "/home/kyan/stage/Datasets/Experiment_results/LLaMA 3-8B-Instruct/v1_fewshot_results_LLaMA 3-8B-Instruct/5runs"
N_RUNS       = 5
...
# ── Build few-shot prompt (V1: no error_type) ──────────────────────────────────
...
# ── Load training set and sample few-shot examples (once, fixed seed) ─────────
with open(TRAIN_FILE, "r", encoding="utf-8") as f:
    train_data = json.load(f)

random.seed(RANDOM_SEED)
shot_examples = random.sample(train_data, N_SHOTS)
print(f"Few-shot examples selected (ids): {[ex['id'] for ex in shot_examples]}")
...
for run in range(5, N_RUNS + 1):
    ...
    output_file = os.path.join(OUTPUT_DIR, f"v1_fewshot_run_{run}.json")
